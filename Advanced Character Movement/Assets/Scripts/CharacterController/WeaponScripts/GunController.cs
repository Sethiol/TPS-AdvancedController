...
public class GunController : MonoBehaviour
{
    ...
    private void FireBullet()
    {
        ...
        if (Physics.Raycast(ray, out hitInfo) && hitInfo.collider.tag != "Player")
        {
            ...
            var rb2d2 = hitInfo.collider.GetComponent<HitBox>();
            if (rb2d2)
            {
               rb2d2.OnRaycastHit(this, ray.direction);
            }
            else
            {
    ...
    void CalculateAxeHit()
    {
        ...
        {
            if(collider.GetComponent<TreeDamageable>() != null)
            {
                collider.GetComponent<TreeDamageable>().Break();
            }
            if(collider.GetComponent<Destructable>() != null)
            {
                collider.GetComponent<Destructable>().DestroyAndReplace();
            }
        }
    }
}
